...
"""
    modified by Sueyeon Kim 19.09.26
"""
...
if sys.version_info >= (2, 6) and platform.system() != "Windows":
    ...
    multiprocessing_enabled = 0  # disabled multiprocessing
    number_of_threads = 4
else:
    ...
    while not EOF:
        ...
        for text, translations in [(src, srctotarget), (target, targettosrc)]:
            ...
            for line in text:
                ...
                """f.readline().rstrip() ⟶ f.rstrip() since the parameters are lists, not files here"""
                for i, f in enumerate(translations):
                    translist[i].append(f.rstrip())
...
class Aligner:
    ...
    """
    added function to get 'multialign' from lists
    """
    def get_align(self, sourcelist, targetlist, translist1, translist2):
    ...
    """
    added function to get aligned result, sources and targets
    """
    def get_mainloop(self, sourcelist, targetlist, translist1, translist2):
        if translist1:
            translist = translist1[0]
        else:
            translist = sourcelist
        sources, targets = self.printout(sourcelist, translist, targetlist)
